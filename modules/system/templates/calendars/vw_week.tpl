                      ...
                      {{if $_event->hour == $_hour}}
                        ...
                        <div id="{{$_event->internal_id}}"
                            ...
                            {{if ($_event->type == "rdvfree" || $_event->type == "rdvfull") && !$_event->disabled}}
                              onclick="setClose('{{$_event->start|date_format:"%H:%M:00"}}', '{{$_event->plage.id}}', '{{$_event->start|date_format:"%A %d/%m/%Y"}}', '{{if isset($chir_id|smarty:nodefaults)}}{{$chir_id}}{{/if}}' , {{if isset($_event->plage.consult_id|smarty:nodefaults)}}'{{$_event->plage.consult_id}}'{{else}}null{{/if}}, this );"
