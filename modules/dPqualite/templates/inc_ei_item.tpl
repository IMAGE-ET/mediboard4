{{* $Id: inc_ei_item.tpl 28539 2015-06-08 13:30:16Z armengaudmc $ *}}
      ...
      <td>
        <select name="ei_categorie_id" class="{{$item->_props.ei_categorie_id}}">
          ...
          {{foreach from=$listCategories item=curr_cat}}
            <option
              value="{{$curr_cat->ei_categorie_id}}"{{if $curr_cat->ei_categorie_id==$item->ei_categorie_id}} selected="selected"{{/if}}>
              {{$curr_cat->nom}}
            </option>
          {{/foreach}}
        </select>
