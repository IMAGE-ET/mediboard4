{{* $Id: vw_idx_rpu.tpl 25698 2014-11-04 16:01:31Z nicolasld $ *}}

{{*
 * @package Mediboard
 * @subpackage dPurgences
 * @version $Revision: 25698 $
      ...
      <td style="text-align: right">
        <form name="selView" action="?m=urgences&amp;tab=vw_idx_rpu" method="post">
          ...
          <select name="selAffichage" onchange="this.form.submit();">
            ...
            <option value="presents"           {{if $selAffichage == "presents"          }}selected{{/if}}>Présents</option>
            <option value="prendre_en_charge"  {{if $selAffichage == "prendre_en_charge" }}selected{{/if}}>A PeC</option>
            <option value="annule_hospitalise" {{if $selAffichage == "annule_hospitalise"}}selected{{/if}}>Annulé et Hospit.</option>
          </select>
        </form>
      ...
      <td style="text-align: right">
       ...
       <form name="UHCD-view" action="" method="post">
          <select name="uhcd_affichage" onChange="UHCD.refreshUHCD()">
            ...
            <option value="presents"          {{if $uhcd_affichage == "presents"         }}selected{{/if}}>Présents</option>
            <option value="prendre_en_charge" {{if $uhcd_affichage == "prendre_en_charge"}}selected{{/if}}>A PeC</option>
            <option value="annule"            {{if $uhcd_affichage == "annule"           }}selected{{/if}}>Annulé</option>
          </select>
        </form>
        <a href="#" onclick="MainCourante.legend()" class="button search">Légende</a>
      </td>
