{{* $Id: inc_planning_day.tpl 22609 2014-03-25 17:11:30Z charlyecho $ *}}

{{*
 * @package Mediboard
 * @subpackage dPbloc
 * @version $Revision: 22609 $
...
<tr>
  <th class="narrow">
    ...
    <form name="chg-{{$curr_day}}" action="?m={{$m}}" method="post" onsubmit="return EditPlanning.lockPlages(this);" class="not-printable">
      ...
      {{if $bloc->_canEdit}}
        <button type="button" class="new notext" onclick="EditPlanning.edit('','{{$curr_day}}', '{{$bloc->_id}}');">{{tr}}Edit{{/tr}}</button>
