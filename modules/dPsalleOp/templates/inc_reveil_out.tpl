...
<table class="tbl">
  ...
  {{foreach from=$listOperations key=key item=_operation}}
    ...
    <tr>
      ...
      <td>
        <button class="notext button soins" onclick="showDossierSoins('{{$patient}}','{{$_operation_id}}');">Dossier de soins</button>
        {{if $isImedsInstalled}}
          <button class="labo button notext" onclick="showDossierSoins('{{$patient}}','{{$_operation_id}}','Imeds');">Labo</button>
        {{/if}}
      ...
      <td class="button">
        {{if $can->edit}}
        <form name="editSortieBlocOutFrm{{$_operation_id}}" action="?" method="post">
      ...
      <td class="button">
        {{if $_operation->entree_reveil}}
          ...
          <form name="editEntreeReveilOutFrm{{$_operation_id}}" action="?" method="post">
            <input type="hidden" name="m" value="planningOp" />
            <input type="hidden" name="dosql" value="do_planning_aed" />
            {{mb_key object=$_operation}}
            <input type="hidden" name="del" value="0" />
      ...
      <td class="button">
        <form name="editSortieReveilOutFrm{{$_operation_id}}" action="?" method="post">
          <input type="hidden" name="m" value="planningOp" />
          <input type="hidden" name="dosql" value="do_planning_aed" />
          {{mb_key object=$_operation}}
          <input type="hidden" name="del" value="0" />
      ...
      {{if $use_sortie_reveil_reel}}
        <td class="button">
          <form name="editSortieReveilReelOutFrm{{$_operation_id}}" action="?" method="post">
            <input type="hidden" name="m" value="planningOp" />
            <input type="hidden" name="dosql" value="do_planning_aed" />
            {{mb_key object=$_operation}}
            <input type="hidden" name="del" value="0" />
      ...
      <td>
        <button type="button" class="print notext"
          onclick="printDossier('{{$patient}}', '{{$_operation_id}}')"></button>
      </td>
    </tr>
  {{foreachelse}}
    <tr>
