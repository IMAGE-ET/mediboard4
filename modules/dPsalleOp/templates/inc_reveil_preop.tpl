...
<table class="tbl">
  ...
  {{foreach from=$listOperations item=_operation}}
    ...
    <tr>
      ...
      {{/if}}
      <td>
        <button class="button soins notext" onclick="showDossierSoins('{{$sejour_id}}','{{$_operation_id}}');">
          Dossier séjour
        </button>
        {{if $isImedsInstalled}}
      ...
      <td style="text-align: center;">
        {{if $modif_operation}}
          <form name="editDebutPreopFrm{{$_operation_id}}" action="?" method="post">
      ...
      <td class="button">
        {{if $modif_operation}}
          <form name="editFinPreopFrm{{$_operation_id}}" action="?" method="post">
            <input type="hidden" name="m" value="planningOp" />
            <input type="hidden" name="dosql" value="do_planning_aed" />
            {{mb_key object=$_operation}}
            <input type="hidden" name="del" value="0" />
