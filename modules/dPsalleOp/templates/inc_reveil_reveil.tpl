...
<table class="tbl">
  ...
  <tr>
    ...
    <td>
      <button class="soins button notext" onclick="showDossierSoins('{{$sejour_id}}','{{$_operation_id}}');">Dossier de soins</button>
      {{if $isImedsInstalled}}
        <button class="labo button notext" onclick="showDossierSoins('{{$sejour_id}}','{{$_operation_id}}','Imeds');">Labo</button>
      {{/if}}
      <button type="button" class="injection notext" onclick="Operation.dossierBloc('{{$_operation_id}}', true)">Dossier de bloc</button>
      {{mb_include module=soins template=vw_antecedents_allergies show_atcd=0}}
    </td>
    {{if $use_poste}}
      <td>
    ...
    {{if $personnels !== null}}
    <td>
      <form name="selPersonnel{{$_operation_id}}" action="?" method="post">
    ...
    {{/if}}
    <td>
      <form name="editEntreeReveilReveilFrm{{$_operation_id}}" action="?" method="post">
    ...
    {{if @$modules.brancardage->_can->read}}
      <td>
         <span id="demande_brancard-{{$sejour_id}}">
    ...
    <td class="button">
      {{if $modif_operation}}
      <form name="editSortieReveilReveilFrm{{$_operation_id}}" action="?" method="post">
        <input type="hidden" name="m" value="planningOp" />
        <input type="hidden" name="dosql" value="do_planning_aed" />
        {{mb_key object=$_operation}}
        <input type="hidden" name="del" value="0" />
        ...
        {{else}}
          {{mb_field object=$_operation field=sortie_reveil_possible form=editSortieReveilReveilFrm`$_operation_id` onchange="submitReveil(this.form)"}}
        {{/if}}
    ...
    {{if $use_sortie_reveil_reel}}
      <td class="button">
        <form name="editSortieReveilReelReveilFrm{{$_operation_id}}" action="?" method="post">
          <input type="hidden" name="m" value="planningOp" />
          <input type="hidden" name="dosql" value="do_planning_aed" />
          {{mb_key object=$_operation}}
          <input type="hidden" name="del" value="0" />
