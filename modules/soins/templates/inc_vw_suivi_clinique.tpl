...
<table class="tbl">
  ...
  {{foreach from=$patient->_ref_cp_by_relation item=_correspondants}}
    {{foreach from=$_correspondants item=_correspondant}}
      <tr>
        <td>
          <button class="copy notext" onclick="Correspondant.edit('{{$_correspondant->_id}}', null, afterEditCorrespondant, 1)">{{tr}}CCorrespondantPatient-copy{{/tr}}</button>
        </td>
        <td>
...
<form name="edit-sejour-frm" method="post" action="?"
  ...
  <table class="tbl">
    ...
    <tr>
      ...
      <td>
        <button type="button" class="edit" onclick='Admissions.validerSortie("{{$sejour->_id}}", {{if $sejour->type == "ambu" && "dPplanningOp CSejour sortie_reelle_ambu"|conf:"CGroups-$g"}}false{{else}}true{{/if}}, window.loadSuiviClinique && loadSuiviClinique.curry("{{$sejour->_id}}"));'>
          Autorisation Sortie
        </button>
      </td>
    </tr>
