...
<tr {{if $sejour->recuse == -1}}class="opacity-70"{{/if}}>
  ...
  <td class="text">
    ...
    {{assign var=curr_op value=$sejour->_ref_curr_operation}}
    {{if "dPhospi vue_temporelle infos_interv"|conf:"CGroups-$g" && ($type == "presents" || $type_mouvement ==  "entrees") && $curr_op->_id}}
      <div class="compact" style="padding-top: 5px;">
        {{$curr_op->_datetime_best|date_format:$conf.date}} {{$curr_op->_datetime_best|date_format:$conf.time}} -
        {{mb_value object=$curr_op field=temp_operation}} -
        {{mb_value object=$curr_op field=type_anesth}}
      </div>
    {{/if}}
  </td>
